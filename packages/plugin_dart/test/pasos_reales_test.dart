...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final r = await const EjecutorDelSistema().correr(
        'dart',
        const ['format', '--output=none', 'no/existe/'],
        directorio: raiz.path,
        presupuesto: const Duration(minutes: 2),
      );
      expect(r.codigo, 0);
      ...
      await expectLater(
        _alcance(raiz.path, const ['no/existe/']),
        throwsArgumentError,
      );
    },
  ...
  test(
    'un alcance inexistente lo delata el ARNÉS, no la herramienta',
    () async {
  ...
  test(
    'código limpio y formateado da verde en los dos pasos',
    () async {
  ...
  test(
    'un archivo sin formatear lo encuentra el paso real',
    () async {
  ...
  test(
    'S4 · un archivo que no parsea produce diagnóstico, no salto silencioso',
    () async {
      fuente('roto.dart', 'void main( {\n');
      final o =
          await formato().run(await _alcance(raiz.path, ['lib/'])) as Executed;
      expect(
        o.diagnostics.map((d) => d.ruleId),
        contains('formato/no-parsea'),
        reason:
            'es el criterio de salida S4 de la fase, contra la '
  ...
  test('un sujeto que ya no existe no se invoca ni se certifica, con el '
    ...
    fuente('bien.dart', 'void main() {\n  print(1);\n}\n');
    final o =
        await formato().run(await _alcance(raiz.path, ['lib/', 'no/existe']))
            as Executed;
    expect(o.witness.subjects, [
      'lib/',
  ...
  test(
    'la reconciliación cierra contra la toolchain de verdad',
    () async {
      ...
      File(
        '${raiz.path}/otro/.escondido/tres.dart',
      ).writeAsStringSync('void  main( ){}\n');

      final o =
          await formato().run(await _alcance(raiz.path, ['lib/', 'otro']))
              as Executed;
  ...
  group('EjecutorDelSistema', () {
    test(
      'una herramienta que no está no se lee como «no encontró nada»',
      () async {
        final r = await const EjecutorDelSistema().correr(
          'no-existe-esta-herramienta',
          const [],
          directorio: raiz.path,
    ...
    test(
      'el presupuesto agotado mata el proceso Y espera a que muera',
      () async {
    ...
    test(
      'una salida que no se puede decodificar no se degrada en silencio',
      () async {
        ...
        expect(
          () => const Utf8Decoder().convert(archivo.readAsBytesSync()),
