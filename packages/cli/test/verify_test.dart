...
void main() {
  group('el código de proceso se deriva del estado', () {
    ...
    test('lo no concluyente da 2, y GANA sobre el 1', () async {
      ...
      expect(
        ...
        contains('el mensaje'),
        reason:
            'el hallazgo real se reporta igual; lo que cambia es el '
            'código, que habla del conjunto',
      );
    });

    test('un paso roto da 70, nunca 1', () async {
      final (c, _) = await correr(const [], [
        Paso.rojo('A'),
        Paso('B', lanza: StateError('x')),
      ]);
      expect(c, 70);
    });

    test('un paso roto Y causas no concluyentes concurrentes: 70, y la '
    ...
    test(
      'una cascada que NO SE PUEDE CONSTRUIR da 70, no una excepción',
      () async {
  ...
  group('el esquema de salida', () {
    ...
    test(
      'un error de uso, que no corre ninguna cascada, no tiene runId',
      () async {
        final (_, salida) = await correr(const [
          '--json',
          '--dry-run',
        ], const []);
  ...
  group('el resultado lleva la causa estructurada y el libro', () {
    test(
      'lo no concluyente trae `inconclusiveBecause` y `obligations`',
      () async {
  ...
  group('la acción siguiente solo nombra evidencia presente en `data`', () {
    test(
      'con lo no concluyente, ni banderas ni comandos que no existen',
      () async {
    ...
    test(
      'nombra el motivo real del testigo, y ese motivo está en `data`',
      () async {
        final (_, salida) = await correr(const ['--json'], [Paso.ciego('A')]);
        final doc = lineas(salida).last;
        ...
        final outcomes = data['outcomes']! as Map<String, Object?>;
        final witness =
            (outcomes['A']! as Map<String, Object?>)['witness']!
                as Map<String, Object?>;
        final omitido = (witness['omitted']! as List)
  ...
  group('`--json --quiet` calla el progreso, no los diagnósticos', () {
    test('con un paso verde, sin diagnósticos que callar, deja solo el '
        'resultado', () async {
    ...
    test('con un paso rojo, el diagnóstico bloqueante viaja igual: el '
  ...
  group('un start sin terminal queda declarado en el resultado', () {
    test('la garantía de entrega: si el canal se rompe, el consumidor no '
        'espera', () async {
    ...
    test('un fallo DESPUÉS del terminal no reescribe la historia', () async {
      ...
      expect(
        ...
        isNull,
        reason:
            'el terminal SÍ se entregó: decir lo contrario es falsear '
  ...
  group('toda salida que no sea verde dice qué hacer', () {
    for (final caso in {
      ...
      test('«${caso.key}» trae su acción siguiente', () async {
        ...
        expect(
          ...
          contains('→'),
          reason:
              'detener sin poder decir qué hacer deja a quien lo choca '
              'sin salida (INV-8)',
        );
      });
    }
    ...
    test('alcance MIXTO —un ajeno y una ruta inexistente—: dice qué NO se '
    ...
    test('alcance SANO y todos los pasos ABORTAN: la acción habla del '
      ...
      expect(
        ...
        isNot(contains('Ningún sujeto del alcance es de este stack')),
        reason:
            'no hay ningún sujeto ajeno que nombrar: el alcance es '
  ...
  group('el silencio y el detalle', () {
    ...
    test('el rescate en modo humano TAMBIÉN dice qué hacer', () async {
      ...
      final (c, salida, _) = await invocar(
        const ['verify'],
        const [],
        construir: (_) => Cascada([
          Paso.verde('A'),
  ...
  group('el binario, con su composición de verdad', () {
    ...
    Future<ProcessResult> shipflow(List<String> args) =>
    ...
    test(
      'registra los pasos reales y los corre de verdad',
      () async {
        ...
        expect(
          ...
          'failed',
          reason:
              'los pasos reales tienen que ENCONTRAR el archivo sin '
    ...
    test(
      'un alcance sin archivos del stack se SALTA, y lo dice',
      () async {
        ...
        expect(
          doc['nextAction'],
          contains(
            'Ningún sujeto del alcance es de '
            'este stack',
          ),
    ...
    test(
      'una ruta INEXISTENTE no es «no tuvo nada que hacer»',
      () async {
    ...
    test(
      'una bandera global ANTES del comando no es un comando',
      () async {
    ...
    test(
      'sin argumentos NO sale con éxito',
      () async {
    ...
    test(
      '`--help` explícito SÍ sale con éxito',
      () async {
    ...
    test(
      'la composición real observa el alcance UNA sola vez',
      () async {
        ...
        final espia = _ObservadorQueCuenta(
          ObservadorDeAlcanceDart(directorio: raiz.path),
        );
        final cascada = cascadaPorDefecto(
        ...
        expect(
          ...
          1,
          reason:
              'dos lecturas del árbol pueden diferir, y entonces el '
